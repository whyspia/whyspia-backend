...
import { fetchUserV2TokenPublicFromDB, getMappingListOfWalletToUserToken, getUserTokenWithDisplayName } from './user-v2.service'

const fetchSenderUserPipeline = { $lookup: {
    from: 'userv2', // The name of the UserV2 collection
    localField: 'senderPrimaryWallet', // Field from Emote
    foreignField: 'primaryWallet', // Field from UserV2 to match against
    as: 'senderUser' // Output array field
  }
}

const fetchReceiverUsersPipeline = { $lookup: {
    from: 'userv2', // The name of the UserV2 collection
    localField: 'receiverSymbols', // Field from Emote
    foreignField: 'primaryWallet', // Field from UserV2 to match against
    as: 'receiverUsers' // Output array field
  }
}
...
export async function createEmoteInDB(
  emoteData: Partial<EmoteRequest>,
  bAgentDecidedSendNotifToReceiver: boolean,
  requestingPrimaryWallet: string | null = null,
): Promise<EmoteResponse | null> {
  ...
  try {
    const emoteBuildData = {
      senderPrimaryWallet: emoteData.senderPrimaryWallet as string,
    ...
    // const senderUserDoc = await UserV2Model.findOne({ primaryWallet: createdEmote?.senderPrimaryWallet })
    // const senderUserWithDisplayName = await getUserTokenWithDisplayName(senderUserDoc, requestingPrimaryWallet as string)
    ...
    const senderUser = userWithDisplayNameMap[createdEmote.senderPrimaryWallet]
    const receiverSymbols = createdEmote.receiverSymbols // This could be a single string or an array of strings
    let receiverUsers: any[] = [] // Initialize an array to hold the receiver users
    // If receiverSymbols is an array, iterate over each symbol
    for (const symbol of receiverSymbols) {
      const user = userWithDisplayNameMap[symbol] // Get the user from the map
      if (user) {
        receiverUsers.push(user) // Add the user to the repliedToReceiverUsers array
      }
    }
    ...
    emoteData.receiverSymbols?.forEach(async (receiverSymbol) => {
      ...
      if (shouldWeSendNotif) {
        await createEmoteNotifInDB({ notifType: NOTIF_TYPE.EMOTE, notifDataID: createdEmote._id.toString(), receiverSymbol, initialNotifData: mapEmoteResponse(createdEmote, senderUser, receiverUsers) })
      }
    })

    return mapEmoteResponse(createdEmote, senderUser, receiverUsers)
  } catch (error) {
...
export async function fetchEmoteFromDB({
  emoteID,
  skipContext = false,
  ...
  try {
    ...
    if (!skipContext) {
      const context = await getContextOfEmote(mapEmoteResponse(emoteDoc?.toObject() as any, null, null), null)
      emoteWithContext = { ...emoteDoc?.toObject(), context }
    }

    const resultEmote = skipContext ? emoteDoc?.toObject() : emoteWithContext as any
    ...
    const userWithDisplayNameMap = await getMappingListOfWalletToUserToken([resultEmote], fieldMapping, requestingPrimaryWallet as string)

    const senderUser = userWithDisplayNameMap[resultEmote?.senderPrimaryWallet]
    const receiverSymbols = resultEmote?.receiverSymbols // This could be a single string or an array of strings
    let receiverUsers: any[] = [] // Initialize an array to hold the receiver users
    // If receiverSymbols is an array, iterate over each symbol
    for (const symbol of receiverSymbols) {
      const user = userWithDisplayNameMap[symbol] // Get the user from the map
    ...
    return emoteDoc ? mapEmoteResponse(resultEmote as any, senderUser, receiverUsers) as EmoteResponse : null
  } catch (error) {
...
function getDBAggregatePipelineForContext(context: string, args: any) {
  ...
  if (context === EMOTE_CONTEXTS.NANA) {
    ...
    const pipeline = [
      ...
      fetchSenderUserPipeline,
      fetchReceiverUsersPipeline,
...
export async function fetchAllEmotesFromDB(
  ...
  try {
    ...
    const { skip, limit, orderBy, senderPrimaryWallet, receiverSymbols, sentSymbols, createdAt, context, requestingPrimaryWallet } = options
    ...
    if (context) {
      ...
      emoteDocs = await EmoteModel.aggregate([
        { $match: filterQuery },
        fetchSenderUserPipeline,
        fetchReceiverUsersPipeline,
        { $sort: sortOptions },
    ...
    if (!skipContext) {
      emotesWithContext = await Promise.all(emoteDocs?.map(async (e: any) => {
        ...
        const calculatedContext = context ?? await getContextOfEmote(mapEmoteResponse(e as any, null, null), null)
        return { ...e, context: calculatedContext }
      }))
    }

    const resultEmotes = skipContext ? emoteDocs : emotesWithContext as any[]
    ...
    const userWithDisplayNameMap = await getMappingListOfWalletToUserToken(resultEmotes, fieldMapping, requestingPrimaryWallet)

    // return resultEmotes?.map((doc) => mapEmoteResponse(doc) as EmoteResponse) as EmoteResponse[]
    return resultEmotes.map(doc => {
      const senderUser = userWithDisplayNameMap[doc.senderPrimaryWallet]
      const receiverSymbols = doc.receiverSymbols // This could be a single string or an array of strings
      let receiverUsers: any[] = [] // Initialize an array to hold the receiver users
      // If receiverSymbols is an array, iterate over each symbol
      for (const symbol of receiverSymbols) {
        const user = userWithDisplayNameMap[symbol] // Get the user from the map
      ...
      return mapEmoteResponse(doc, senderUser, receiverUsers) as EmoteResponse
    })
  } catch (error) {
...
export async function fetchUnrespondedEmotesFromDB(
  ...
  const { skip, limit, orderBy, orderDirection, fetchSentOrReceived, requestingPrimaryWallet } = options

  const primaryUserFilter = fetchSentOrReceived === 'sent'
    ?
    {
      $match: {
        senderPrimaryWallet: receiverOrSenderSymbol,
        receiverSymbols: {
          // this is what filters out the additional emotes used for identifying noucontext and replies - prob not ideal tbh 
          $not: {
            $in: [
  ...
  try {
    ...
    const emotesWithReplyChains = await Promise.all(result.map(async (emote) => {
      ...
      const { chain, totalChainLength } = await findEmoteReplyChainInDB(emote._id, { skip: 1, limit: 2, orderBy, orderDirection }, requestingPrimaryWallet) // Adjust skip and limit as needed
      
      const context = await getContextOfEmote(mapEmoteResponse(emote as any, null, null), null)
    ...
    const userWithDisplayNameMap = await getMappingListOfWalletToUserToken(emotesWithReplyChains, fieldMapping, requestingPrimaryWallet)
    ...
    return emotesWithReplyChains.map(doc => {
      const senderUser = userWithDisplayNameMap[doc.senderPrimaryWallet]
      const receiverSymbols = doc.receiverSymbols // This could be a single string or an array of strings
      let receiverUsers: any[] = [] // Initialize an array to hold the receiver users
      // If receiverSymbols is an array, iterate over each symbol
      for (const symbol of receiverSymbols) {
        const user = userWithDisplayNameMap[symbol] // Get the user from the map
        if (user) {
          receiverUsers.push(user) // Add the user to the receiverUsers array
        }
      }
      return mapEmoteResponseWithNoUChainPreviews(doc, senderUser, receiverUsers) as EmoteResponse
    })
  } catch (error) {
...
export async function findEmoteReplyChainInDB(
  emoteId: string,
  options: EmoteNouChainQueryOptions,
  requestingPrimaryWallet: string | null = null,
): Promise<{ chain: EmoteResponse[], totalChainLength: number }> {
  ...
  // const senderUserDoc = await UserV2Model.findOne({ primaryWallet: currentEmote?.senderPrimaryWallet })
  // const senderUserWithDisplayName = await getUserTokenWithDisplayName(senderUserDoc, requestingPrimaryWallet as string)
  // get userTokens for each wallet (also handles calculatedDisplayName of each user relative to requestingPrimaryWallet)
  const fieldMapping = {
    senderPrimaryWallet: 'senderUser',
  ...
  const userWithDisplayNameMap = await getMappingListOfWalletToUserToken([currentEmote], fieldMapping, requestingPrimaryWallet as string)
  const currentEmoteSenderUser = userWithDisplayNameMap[currentEmote.senderPrimaryWallet]
  const currentEmoteReceiverSymbols = currentEmote.receiverSymbols // This could be a single string or an array of strings
  let currentEmoteReceiverUsers: any[] = [] // Initialize an array to hold the receiver users
  // If currentEmoteReceiverSymbols is an array, iterate over each symbol
  for (const symbol of currentEmoteReceiverSymbols) {
    const user = userWithDisplayNameMap[symbol] // Get the user from the map
    if (user) {
      currentEmoteReceiverUsers.push(user) // Add the user to the currentEmoteReceiverUsers array
    }
  }

  const chain: any[] = [mapEmoteResponse({ ...currentEmote.toObject(), context: EMOTE_CONTEXTS.NOU }, currentEmoteSenderUser, currentEmoteReceiverUsers)] as any
  let searching = true
  ...
  while (searching) {
    ...
    if (repliedToEmote) {
      ...
      const userWithDisplayNameMap = await getMappingListOfWalletToUserToken([repliedToEmote], fieldMapping, requestingPrimaryWallet as string)
      const repliedToEmoteSenderUser = userWithDisplayNameMap[repliedToEmote.senderPrimaryWallet]
      const repliedToEmoteReceiverSymbols = repliedToEmote.receiverSymbols // This could be a single string or an array of strings
      let repliedToEmoteReceiverUsers: any[] = [] // Initialize an array to hold the receiver users
      // If repliedToEmoteReceiverSymbols is an array, iterate over each symbol
      for (const symbol of repliedToEmoteReceiverSymbols) {
        const user = userWithDisplayNameMap[symbol] // Get the user from the map
        if (user) {
          repliedToEmoteReceiverUsers.push(user) // Add the user to the repliedToReceiverUsers array
        }
      }
      
      chain.unshift(mapEmoteResponse({ ...repliedToEmote.toObject(), context: EMOTE_CONTEXTS.NOU } as any, repliedToEmoteSenderUser, repliedToEmoteReceiverUsers)) // Prepend to maintain order
